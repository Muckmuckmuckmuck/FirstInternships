...
export const api = {
  ...
  async getProfile() {
    ...
    if (!data) return null;
    // Read-then-reset (server-time authoritative). Free = 5/day, Pro = 1,000/month.
    // Resets regardless of whether the previous allowance was used up.
    const today = new Date().toISOString().slice(0, 10);   // YYYY-MM-DD
    const month = today.slice(0, 7);                        // YYYY-MM
    const patch = {};
    if (data.plan === "pro") {
      if (String(data.cycle_start || "").slice(0, 7) !== month) { patch.credits = 1000; patch.cycle_start = today; }
    } else {
      if (data.daily_date !== today) { patch.credits = 5; patch.daily_date = today; }
    }
