...
export const api = {
  ...
  // ── BILLING ──────────────────────────────────────────────────────────────────
  async upgradeToPro() { const r = await fetch("/api/stripe-checkout", { method: "POST", headers: await authHeader() }); const d = await r.json(); if (!r.ok || !d.url) throw new Error(d.error || "checkout_failed"); window.location.href = d.url; },
  async buyTopup(qty) { const r = await fetch("/api/stripe-topup", { method: "POST", headers: { "Content-Type": "application/json", ...(await authHeader()) }, body: JSON.stringify({ qty }) }); const d = await r.json(); if (!r.ok || !d.url) throw new Error(d.error || "topup_failed"); window.location.href = d.url; },
