...
export const api = {
  ...
  async listFirms({ search = "", industry = "All", limit = 20000 } = {}) {
    ...
    const PAGE = 1000;
    const build = () => {
      // active=false is the suppression gate: opt-out requests and known-stale
      // sources stay in the table (so they can't be silently re-imported) but are
      // never served to a user.
      let q = sb.from("firms").select("*", { count: "exact" }).eq("active", true);
      if (industry !== "All") q = q.eq("industry", industry);
      if (search) q = q.ilike("name", `%${search}%`);
      // ORDER BY is REQUIRED for correct pagination. Postgres gives no ordering
      // guarantee without it, so paging with .range() can return the same row twice
      // and skip others entirely — which silently hid whole slices of the table.
  ...
  // ── PIPELINE / CONTACTS ─────────────────────────────────────────────────────
  async listContacts() {
    if (!sb) return [];   // contract: always an array of contact rows (consumer calls .forEach)
    const u = await this.getSession(); if (!u) return [];
  ...
  async generateEmail({ firm, profile, level, resume, tone, length, emphasis }) {
    ...
    // Throw on any non-2xx (e.g. the daily draft cap → 429) so the caller can fall
    // back to the local template instead of silently clearing the draft to empty.
    if (!r.ok) throw new Error((await r.json().catch(() => ({}))).error || "generate_failed");
  ...
  // ── BILLING ──────────────────────────────────────────────────────────────────
  // All three Stripe redirects share one endpoint (see api/stripe.js).
  async _stripe(action, body = {}) {
    ...
    const d = await r.json().catch(() => ({}));
    if (!r.ok || !d.url) throw new Error(d.error || `${action}_failed`);
    window.location.href = d.url;
  },
  async upgradeToPro()      { return this._stripe("checkout"); },
  async buyTopup(qty)       { return this._stripe("topup", { qty }); },
  async manageSubscription(){ return this._stripe("portal"); },

  // ── PUBLIC OPT-OUT ──────────────────────────────────────────────────────────
  // No auth: a recruiter who wants out shouldn't have to create an account first.
  // The server always replies the same way whether or not the address was found, so
  // this can't be used to probe who is in the directory.
  async removeEmail(email) {
    const r = await fetch("/api/remove-email", {
      method: "POST", headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ email }),
    });
    const d = await r.json().catch(() => ({}));
    if (!r.ok) throw new Error(d.message || d.error || "removal_failed");
    return d;
  },
