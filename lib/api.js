...
export const api = {
  ...
  async connectGmail() {
    ...
    const r = await fetch("/api/gmail-oauth-url", { method: "POST", headers: { Authorization: `Bearer ${token}` } });
  ...
  async gmailStatus() {
    ...
    try {
      const r = await fetch("/api/gmail-status", { headers: await authHeader() });
  ...
  async saveResume({ file, text }) {
    ...
    if (file) {
      const storage_path = `${u.id}/${file.name}`;
      await sb.storage.from("resumes").upload(storage_path, file, { upsert: true });
      patch.storage_path = storage_path;
      patch.file_name = file.name;
    }
    await sb.from("resumes").upsert(patch);   // omitting storage_path on text-only save preserves the current file
    return patch;
  },
